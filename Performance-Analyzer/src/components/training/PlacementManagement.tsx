...
const PlacementManagement = () => {
  ...
  const handleViewEligibleStudents = async (id: string, minScore?: number) => {
    setIsLoadingEligible(true);
    setIsEligibleDialogOpen(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/jobs/${id}/eligible-students`);
      if (response.ok) {
        setEligibleStudents(await response.json());
      } else {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
            ...
            <TableRow key={job.id}>
              ...
              <TableCell className="text-right flex items-center justify-end gap-1">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleViewEligibleStudents(job.id, job.min_score)}
                  title="View Eligible Students"
                >
                  <Users size={14} className="mr-1" /> Eligible Students
                </Button>
                <Button
                  variant="ghost"
                  size="icon"
      ...
      <Dialog open={isEligibleDialogOpen} onOpenChange={setIsEligibleDialogOpen}>
        <DialogContent>
          <DialogHeader>
          ...
          <div className="mt-4 max-h-[60vh] overflow-y-auto">
            {isLoadingEligible ? (
              <p className="text-center text-muted-foreground animate-pulse py-4">Loading eligible students...</p>
            ) : eligibleStudents.length === 0 ? (
              <p className="text-center text-muted-foreground py-4">No students meet the eligibility criteria.</p>
            ) : (
              <div className="space-y-2">
                <div className="bg-secondary/20 p-2 rounded text-sm text-center font-medium mb-4">
                  Total Eligible: {eligibleStudents.length}
                </div>
                {eligibleStudents.map((s, idx) => (
                  <div key={idx} className="flex justify-between items-center p-3 border border-border/50 rounded-lg hover:bg-secondary/5">
                    <div>
                      <p className="font-medium text-sm">{s.name}</p>
                      <p className="text-xs text-muted-foreground">{s.rollNumber}</p>
                    </div>
                    <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded">Section {s.section}</span>
                  </div>
                ))}
              </div>
            )}
